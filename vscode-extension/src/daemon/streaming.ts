...
import { _cfg, _errorMessage } from "./transport";
import { DaemonError } from "./types";
import type { TransformResult, StreamStageEvent } from "./types";
import { transformText } from "./core";
...
export async function transformTextStream(
  ...
  onProgress: (event: StreamStageEvent) => void,
  signal?: AbortSignal
  ...
  while (true) {
    ...
    for (const frame of frames) {
      ...
      let event: StreamStageEvent;
      try {
        event = JSON.parse(json) as StreamStageEvent;
      } catch {
        continue;
      }
