...
exports.medRiskDecoration = exports.highRiskDecoration = void 0;
exports.registerSectionCommands = registerSectionCommands;
exports.applyDecorationsForActiveEditor = applyDecorationsForActiveEditor;
exports.getSectionProvider = getSectionProvider;
...
// ---------------------------------------------------------------------------
exports.highRiskDecoration = vscode.window.createTextEditorDecorationType({
    backgroundColor: "rgba(220, 50, 50, 0.18)",
    isWholeLine: false,
    overviewRulerColor: "rgba(220, 50, 50, 0.5)",
    overviewRulerLane: vscode.OverviewRulerLane.Right,
});
exports.medRiskDecoration = vscode.window.createTextEditorDecorationType({
...
function _cfg() {
    ...
    return {
        ...
        autoScore: cfg.get("autoScore", true),
        binaryPath: cfg.get("binaryPath", "humanize"),
    };
}
function _log(msg) {
 ...
 */
function _findSectionAtCursor(nodes, cursorLine, includeShort = false) {
 ...
 */
function _extractSectionText(document, node) {
    const lines = [];
 ...
 */
async function _replaceSectionText(editor, node, newText) {
...
async function _applyDecorations(editor) {
    ...
    for (let i = 0; i < eligible.length; i++) {
        ...
        if (result.status !== "fulfilled") {
            _log(`Decoration scoring failed for paragraph at line ${eligible[i].startLine}: ${String(result.reason)}`);
            continue;
        }
        const score = result.value.score;
        const range = new vscode.Range(new vscode.Position(eligible[i].startLine, 0), new vscode.Position(eligible[i].endLine, editor.document.lineAt(eligible[i].endLine).text.length));
        if (score >= 0.67) {
    ...
    editor.setDecorations(exports.highRiskDecoration, highRanges);
    editor.setDecorations(exports.medRiskDecoration, medRanges);
}
/**
 * Clear all decorations from the editor.
 */
function _clearDecorations(editor) {
    editor.setDecorations(exports.highRiskDecoration, []);
    editor.setDecorations(exports.medRiskDecoration, []);
}
/**
 * Split the document into paragraph blocks (separated by blank lines).
 */
function _splitIntoParagraphs(_text, document) {
    const paragraphs = [];
    let blockStart = -1;
    let blockLines = [];
...
// ---------------------------------------------------------------------------
function _progressKey(title) {
    return title.toLowerCase().trim();
}
function _saveNodeProgress(ctx, node) {
    const data = (0, progressStore_1.loadProgress)(ctx);
    ...
    (0, progressStore_1.saveProgress)(ctx, data);
}
// ---------------------------------------------------------------------------
// registerSectionCommands — the single entry point extension.ts calls.
// ---------------------------------------------------------------------------
function registerSectionCommands(ctx) {
    ...
    const sectionProvider = new sectionProvider_1.SectionProvider(ctx);
    _sectionProvider = sectionProvider;
    // Register the tree data provider for the humanizer.sections view
    ctx.subscriptions.push(vscode.window.registerTreeDataProvider("humanizer.sections", sectionProvider));
    ...
    ctx.subscriptions.push(vscode.workspace.onDidSaveTextDocument(async (document) => {
        const cfg = _cfg();
        if (!cfg.autoScore) {
            return;
        }
    ...
    ctx.subscriptions.push(vscode.commands.registerCommand("humanizer.scoreSection", async () => {
        ...
        if (!found) {
            vscode.window.showWarningMessage("Cursor is not inside a scoreable section. Move to a section body.");
            return;
        }
        ...
        try {
            const cfg = _cfg();
            const result = await (0, daemonClient_1.scoreText)(text, cfg.profile);
            ...
            await _applyDecorations(editor);
            vscode.window.setStatusBarMessage(`Section "${node.title}": ${result.score.toFixed(2)} ${result.band.toUpperCase()}`, 5000);
        }
        catch (err) {
            _showError(err);
        }
    ...
    ctx.subscriptions.push(vscode.commands.registerCommand("humanizer.transformSection", async () => {
        ...
        const freshNodes = (0, sectionProvider_1.parseHeadings)(lines);
        const found = _findSectionAtCursor(freshNodes, cursorLine);
        if (!found) {
            vscode.window.showWarningMessage("Cursor is not inside a transformable section. Move to a section body (must be > 30 words and not the References section).");
            return;
        }
        const { node } = found;
        const nodeIndex = sectionProvider.nodes.findIndex((n) => n.lineStart === node.lineStart);
        const text = _extractSectionText(editor.document, node);
        if (text.trim().length === 0) {
    ...
    ctx.subscriptions.push(vscode.commands.registerCommand("humanizer.transformAll", async () => {
        ...
        const pending = allNodes
            ...
            .filter(({ node }) => node.status === "pending" && node.wordCount >= 30);
        if (pending.length === 0) {
            vscode.window.showInformationMessage("No pending sections to transform. All sections are done, skipped, or too short.");
            return;
        }
        let doneCount = 0;
        ...
        await vscode.window.withProgress({
            ...
            for (let i = 0; i < pending.length; i++) {
                ...
                const freshNodes = (0, sectionProvider_1.parseHeadings)(freshLines);
                const freshNode = freshNodes.find((n) => n.title === node.title && n.level === node.level);
                if (!freshNode) {
        ...
        try {
            await vscode.window.withProgress({
                ...
                try {
                    ...
                    await (0, daemonClient_1.exportDocxToFile)(result.output, outputPath);
                    advance(100, "done");
                }
                catch (daemonErr) {
                    // Only fall back when the daemon is unreachable (status 0).
                    // Any other error (401, 502, etc.) is a real error — rethrow.
                    if (!(daemonErr instanceof daemonClient_1.DaemonError) || daemonErr.status !== 0) {
                        throw daemonErr;
                    }
                    usedDaemon = false;
                    advance(10, "daemon offline — using local binary…");
                    await new Promise((resolve, reject) => {
                        (0, child_process_1.execFile)(cfg.binaryPath, ["transform", inputPath, "--stages", "prescan,determ,postscan", "--out", outputPath], { env: process.env }, (error, _stdout, stderr) => {
                            if (error) {
    ...
    ctx.subscriptions.push(vscode.commands.registerCommand("humanizer.importReview", async () => {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Humanizer: Could not read DOCX file — ${String(err)}`);
            return;
        }
        // Step 4: call the daemon
        let result;
        try {
            result = await (0, daemonClient_1.reviewImport)(docxBase64, originalText);
        }
        catch (err) {
            _showError(err);
            return;
        ...
        channel.appendLine(`Changed sections: ${changedCount} / ${result.diff_sections.length}`);
        channel.appendLine(`Post-import score: ${result.post_score.score.toFixed(2)} (${result.post_score.band.toUpperCase()})`);
        ...
        if (changedCount > 0) {
            ...
            for (const sec of result.diff_sections.filter((s) => s.changed)) {
                ...
                channel.appendLine(`    Original: ${sec.original.slice(0, 80).replace(/\n/g, " ")}…`);
                channel.appendLine(`    Revised:  ${sec.revised.slice(0, 80).replace(/\n/g, " ")}…`);
            }
        }
        channel.show(true);
        vscode.window.showInformationMessage(`Humanizer: Review import done — ${changedCount} changed section(s), ` +
...
function applyDecorationsForActiveEditor() {
    ...
    if (editor && editor.document.languageId === "markdown") {
        _applyDecorations(editor).catch((err) => {
            ...
            if (_outputChannel) {
                _outputChannel.appendLine(`[Humanizer] Decoration error: ${String(err)}`);
            }
        });
    }
}
// Expose a getter for the SectionProvider singleton (used by extension.ts if needed)
function getSectionProvider() {
    return _sectionProvider;
}
//# sourceMappingURL=sectionProcessor.js.map