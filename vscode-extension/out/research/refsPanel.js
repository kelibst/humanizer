...
exports.handleRefsMessage = handleRefsMessage;
...
function _errMsg(err) {
    return err instanceof daemonClient_1.DaemonError
        ? err.message
...
// Individual handlers
// ---------------------------------------------------------------------------
async function _handleList(webview) {
...
async function _handleDelete(id, webview) {
    const root = _workspaceRoot();
    if (!root || !id) {
        return;
    }
    try {
        await (0, daemonClient_1.deleteRef)(root, id, _activeDocPath());
...
async function _handleCopyCitationKey(id) {
    const root = _workspaceRoot();
    if (!root || !id) {
        return;
    }
    try {
        ...
        const key = (0, insertCitation_1.citationKey)(ref);
...
async function _handleImportBibtex(webview) {
    ...
    try {
        ...
        vscode.window.showErrorMessage(`Humanizer: could not read file — ${_errMsg(err)}`);
        return;
    }
...
async function _handleExportBibtex(webview) {
    const root = _workspaceRoot();
    if (!root) {
    ...
    try {
        ...
        vscode.window.showErrorMessage(`Humanizer: BibTeX export failed — ${_errMsg(err)}`);
        webview.postMessage({ type: "refs.error", message: _errMsg(err) });
        return;
    }
    ...
    try {
        ...
        vscode.window.showErrorMessage(`Humanizer: could not write file — ${_errMsg(err)}`);
    }
}
// ---------------------------------------------------------------------------
// Message router
// ---------------------------------------------------------------------------
async function handleRefsMessage(msg, webview) {
    switch (msg.type) {
        case "refs.list":
...
//# sourceMappingURL=refsPanel.js.map