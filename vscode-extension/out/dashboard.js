...
function openDashboard(ctx) {
    ...
    _panel = vscode.window.createWebviewPanel("humanizer.dashboard", "Research Dashboard", vscode.ViewColumn.One, {
        enableScripts: true,
...
async function _kickRefresh() {
    ...
    await Promise.all([
        (async () => {
            try {
                checklist = await (0, daemonClient_1.checklist)(text, profile);
            }
            catch (err) {
        ...
        (async () => {
            try {
                readabilityRes = await (0, daemonClient_1.readability)(text, profile);
            }
            catch (err) {
        ...
        (async () => {
            try {
                if (wsRoot) {
                    citationsRes = await (0, daemonClient_1.citations)(text, wsRoot, profile);
                }
            }
            catch (err) {
        ...
        (async () => {
            try {
                const r = await (0, daemonClient_1.scoreText)(text, profile);
    ...
    const history = ctx.workspaceState.get(HISTORY_KEY, {});
    ...
    const wordsPerSectionTarget = readabilityRes?.targets.wordsPerSection?.target ?? null;
    _panel.webview.postMessage({
        type: "data",
...
function _workspaceRootFor(uri) {
