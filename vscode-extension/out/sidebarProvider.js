...
class SidebarProvider {
    ...
    // Handlers
    // -------------------------------------------------------------------------
    async _onReady(webview) {
        const cfg = vscode.workspace.getConfiguration("humanizer");
        const backend = cfg.get("backend", "ollama");
        webview.postMessage({
            type: "config",
            settings: {
                daemonUrl: cfg.get("daemonUrl", "https://localhost:9999"),
                profile: cfg.get("profile", "default_ghanaian"),
                backend,
                autoScore: cfg.get("autoScore", true),
                includeLlm: cfg.get("includeLlm", false),
    ...
    _buildHtml(webview) {
        ...
        const cssUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, "src", "webview", "sidebar.css"));
